...
interface MyDataUserInputProps {
  history: any;
}

export default function MyDataUserInput({ history }: MyDataUserInputProps) {
  ...
  const handlePhoneChange = (
    ...
    if (part === "phone1") setPhone1(numericValue);
    else if (part === "phone2") setPhone2(numericValue);
    else if (part === "phone3") setPhone3(numericValue);
  ...
  const handleSSNChange = (
    ...
    if (part === "ssn1") setSsn1(numericValue);
    else if (part === "ssn2") setSsn2(numericValue);
  ...
  const sendVerification = useCallback(async () => {
    ...
    stopTimer();
    const { success } = await certifyUserCiMutation({
      userName,
      userPhone: `${phone1}-${phone2}-${phone3}`,
      userNum: `${ssn1}-${ssn2}`,
      verifyCode,
  ...
  return (
    <>
      ...
      {currentStep === 3 && (
        <div className="px-6">
          <Button
            className="w-full mx-auto text-lg h-16 mb-10"
            onClick={handleGetVerification}
            disabled={!isPhoneComplete || !selectedCarrier}
          >
            인증번호 요청
