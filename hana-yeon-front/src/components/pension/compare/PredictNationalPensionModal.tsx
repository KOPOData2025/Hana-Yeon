...
interface PredictNationalPensionModalProps {
  goBack?: () => void;
  onSelect?: (amount: number) => void;
}

export default function PredictNationalPensionModal({
  goBack,
  onSelect,
}: PredictNationalPensionModalProps) {
  ...
  const handleSelectPension = (amount: number) => {
    onSelect?.(amount);
  };
  ...
  return (
    <div className="min-h-screen p-4 relative">
      <div className="flex items-center">
        <button onClick={goBack} className="p-1 z-9999">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-4 pt-4">
            <div className="space-y-3">
                ...
                {rest.map(([years, amount]) => (
                  <button
                    ...
                    onClick={() =>
                      pensionResults &&
                      handleSelectPension(
                        pensionResults[years as keyof typeof pensionResults]
                      )
                    }
              ...
              <button
                onClick={() =>
                  pensionResults && handleSelectPension(pensionResults[40])
                }
                className="w-full p-3 bg-olo/30 dark:bg-olo/60 border border-gray-400 rounded-md text-center"
              >
                <div className="text-xs text-gray-500 dark:text-gray-300 mb-1">
                  {last?.[0]} 가입
                </div>
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed pt-5">
              ...
              같습니다.
              <br />
              국민연금 급액(국민연금 1365)로 연락하여 상세내역(11년 가입자
              소득특례, 크레딧 등) 확인하여 더정확한 상담이 가능합니다.
              <br />
              상세내역 중 보험료 자격득실 등을 상담받고 지급예상연금액을 변경할
              수 있습니다.
              <br />
              지급예상연금액은 세전금액으로 향후 연금수급 시 세금을 공제하고
              급액 중 있습니다.
            </div>
          </div>
        </div>
