package com.hanati.hanabank_backend.domain.account.controller;
...
import com.hanati.hanabank_backend.domain.account.service.AccountService;
...
public class AccountController {
    ...
    public ResponseEntity<ApiResponse<CreateAccountResponse>> createAccount(@Valid @RequestBody CreateAccountRequest request) {
        log.info("계좌 생성 요청 - CI: {}, 상품명: {}, 계좌타입: {}", 
                request.getUserCi(), request.getProductName(), request.getAccountType());
        try {
            CreateAccountResponse response = accountService.createAccount(request);
            log.info("계좌 생성 성공 - 계좌번호: {}", response.getAccountNum());
            return ResponseEntity.ok(ApiResponse.success(response));
        } catch (IllegalArgumentException e) {
            log.warn("계좌 생성 실패: {}", e.getMessage());
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            log.error("계좌 생성 중 오류 발생", e);
            return ResponseEntity.internalServerError()
