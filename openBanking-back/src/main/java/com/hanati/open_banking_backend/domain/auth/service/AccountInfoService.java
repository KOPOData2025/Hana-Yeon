package com.hanati.open_banking_backend.domain.auth.service;
...
public class AccountInfoService {
    ...
    public AccountInfoResponse getAccountList(AccountInfoRequest request) {
        ...
        log.info("계좌통합조회 요청 수신 - userName: {}, inquiryBankType: {}, inquiryRecordCnt: {}", 
                request.getUserCi(), request.getInquiryBankType(), request.getInquiryRecordCnt());
        ...
        // 은행 API URL 목록 생성 (단일 값을 배열로 변환)
        List<String> bankApiUrls = Arrays.asList(
            hanaBankBaseUrl + "/api/v1/accounts/search",
            shinhanBankBaseUrl + "/api/v1/accounts/search"
        );
        
        // 은행 API 호출
        for (String apiUrl : bankApiUrls) {
            try {
                List<AccountInfoResponse.AccountInfo> accounts = fetchAccountsFromExternalApi(apiUrl, request.getUserCi(), request.getUserCi());
                allAccounts.addAll(accounts);
                log.info("은행 API 조회 성공 - URL: {}, 조회 건수: {}", apiUrl, accounts.size());
            } catch (Exception e) {
                log.warn("은행 API 조회 실패 - URL: {}, 오류: {}", apiUrl, e.getMessage());
